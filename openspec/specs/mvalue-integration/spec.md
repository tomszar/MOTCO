# mvalue-integration Specification

## Purpose
Ensure that methylation data is always converted from B-value (beta) space to M-value (logit) space before it enters any integration processing, so that integration operates on a linear, direction-preserving representation. Leaving methylation in B-value space at the integration step manufactures magnitude→orientation cross-talk, because the sigmoid nonlinearity breaks direction-preservation (`sigmoid(c·a) ≠ c·sigmoid(a)`). This capability defines the contract that integration always operates in the generator's native M-value space.
