...
- **Integration-space contrast (β vs M-value):** add an `integration_space` knob (default `"mvalue"`) selecting whether the projector integrates the carried β values directly or a clipped `logit` (M-value) of them, and contrast the two. Since `logit` is the exact inverse of the generative `rev_logit`, M-value integration — the standard analysis practice — recovers the clean linear geometry, removing both the compression and the cross-talk. This reframes the gate: methylation `rev.logit` is a β-representation artifact, not a standing cross-talk source for a correctly-built (M-integrating) pipeline.
