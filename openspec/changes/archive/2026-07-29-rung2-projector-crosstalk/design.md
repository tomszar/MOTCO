...
**3. PLS-DA headline label is the *stage*; a group-conditioned `none` null is the leakage control.**
PLS-DA is supervised and needs a class label. In the production trajectory pipeline (`viz.plot_trajectory_from_plsr`, used by the showcase) the PLS-DA is fit with the **stage** factor as the response: the latent axes are oriented to separate disease stages — i.e. to trace the trajectory itself — and group A-vs-B differences are then measured *within* that space. The headline `plsda` arm therefore conditions on stage (`plsda_label="stage"`, the default), matching how MOTCO actually builds its trajectory latent space. Conditioning on the *group* (A/B) is a different, adversarial question — it is the worst-case supervised-leakage configuration, where PLS-DA can manufacture a group separation even when the two groups have identical (`none`) trajectories. That configuration is retained exclusively inside `run_leakage_probe` (which forces `plsda_label="group"`): under an honest projector `none` stays at the null floor; under label leakage the group-conditioned PLS-DA inflates it. (Earlier drafts made group the headline on the premise that group is "the production classification target"; that premise is wrong for the trajectory-geometry path, whose production target is the stage.)
